...
const ytmp4 = async (url, options = {}) => {
    try {
        const videoQuality = options.videoQuality || '360';
        // මෙතනට ඔයා පාවිච්චි කරන ඕනෑම YouTube API එකක් දාන්න පුළුවන්
        // උදාහරණයක් විදියට පබ්ලික් API එකක්:
        const response = await fetchJson(`https://api.giftedtech.my.id/api/download/ytmp4?url=${encodeURIComponent(url)}&apikey=gifteddev`);
...
module.exports = { 
    ...
    saveConfig,
    empiretourl,
    ytmp4 // මේක අනිවාර්යයෙන්ම මෙතනට එකතු කරන්න
};
